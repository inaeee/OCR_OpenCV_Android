...
import static com.example.opencv_camera.MainActivity.sTess;
...
public class CameraView extends Activity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mOrientEventListener = new OrientationEventListener(this,
            ...
            public void onOrientationChanged(int arg0) {
                //방향센서값에 따라 화면 요소들 회전
                // 0˚ (portrait)
                //기기의 회전상태에 따라 각각의 뷰들이 함께 회전한다.
                //portrait와 landscape를 제외한 나머지 둘 90, 180도는 TextView가 어딘가에 가려진 듯 보이지 않는다.
                if (arg0 >= 315 || arg0 < 45) {
                    rotateViews(270);
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_ocrstart:
                if (!mStartFlag) {
                    ...
                    //Bitmap.createBitmap 사진에 필요한 부분만 잘라온다
                    //Start버튼을 누르면 Rect형식으로 지정된 ROI영역을 Mat형식으로 가져오게 되고, 그것을 Bitmap으로 전환시켜
                    //AsyncTask객체로 Tesseract OCR 작업을 한다. 일반 함수를 쓰면 처리를 하는 동안 카메라 프레임이 멈춘다.
                    ...
                    if (mCurrOrientHomeButton != mOrientHomeButton.Right) {
                        switch (mCurrOrientHomeButton) { //기기의 회전 상태에 따라 인식될 비트맵도 함께 회전한다.
                            case Bottom:
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        // ROI 영역 흑백으로 전환 ,ROI 영역은 글자 인식 영역이므로 흑백 처리를 한다.
